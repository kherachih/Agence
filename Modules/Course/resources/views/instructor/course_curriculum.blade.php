...
@section('body-content')
                                            ...
                                            @foreach ($course_modules as $index => $course_module)
                                                <tr class="odd">
                                                    ...
                                                    <td class="crancy-table__column-2 crancy-table__data-2 edc_gropu_action_btns">
                                                        <!-- actions -->
                                                        <a
                                                           href="{{ route('instructor.course-lesson', ['course_id' => $course->id, 'course_module' => $course_module->id, 'req_type' => 'from_create'] ) }}"
                                    ...
                                    <div class="d-flex justify-content-end">
                                        <div>
                                            @if (request()->has('req_type') && request()->get('req_type') == 'from_create')
                                                ...
                                                <a class="crancy-btn  next-btn mg-top-25" href="{{ route('instructor.course-media', ['course_id' => $course->id, 'req_type' => 'from_create'] ) }}">  {{ __('translate.Previous') }}</a>

                                                <a class="crancy-btn edc-crs-step-save-btn  next-btn p mg-top-25" href="{{ route('instructor.course-seo', ['course_id' => $course->id, 'req_type' => 'from_create']) }}">  {{ __('translate.Next') }}</a>

                                            @else

                                                <a class="crancy-btn  next-btn mg-top-25" href="{{ route('instructor.course-media', $course->id ) }}">  {{ __('translate.Previous') }}</a>
        ...
        <div class="modal fade" id="curriculumEdit{{ $course_module->id }}" tabindex="-1"
            ...
            <div class="modal-dialog modal-lg">
                <form class="modal-content delet_modal_form" method="POST"
                      action="{{ route('instructor.update-course-curriculum', ['course_id' => $course->id, 'course_module' => $course_module->id]) }}">
                    @csrf
                    @method('PUT')
    ...
    <div class="modal fade" id="curriculumAdd" tabindex="-1" aria-labelledby="curriculumAdd" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <form class="modal-content delet_modal_form" method="POST"
                  action="{{ route('instructor.store-course-curriculum', $course->id) }}">
                @csrf
                <div class="modal-header">
...
@push('js_section')
    <script>
        ...
        function itemDeleteConfrimation(course_id, course_module_id) {
            ...
            $("#item_delect_confirmation").attr("action", '{{ url("instructor/destroy-course-curriculum/") }}' + "/" + course_id + "/" + course_module_id)
        }
    </script>
@endpush
