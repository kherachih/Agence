...
@section('title')
    <title>{{ __('translate.Add New Continent') }}</title>
@endsection

@section('body-header')
    <h3 class="crancy-header__title m-0">{{ __('translate.Add New Continent') }}</h3>
    <p class="crancy-header__text">{{ __('translate.Tour Booking') }} >> {{ __('translate.Add New Continent') }}</p>
@endsection
                            ...
                            <div class="crancy-table crancy-table--v3 mg-top-30">
                                <div class="crancy-customer-filter">
                                    <div class="crancy-header__form crancy-header__form--customer create_new_btn_inline_box">
                                        <h4 class="crancy-product-card__title">{{ __('translate.Add New Continent') }}</h4>
                                ...
                                <form action="{{ route('admin.tourbooking.continents.store') }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group mb-3">
                                                ...
                                                <input type="text" name="icon" id="icon" class="form-control @error('icon') is-invalid @enderror"
                                                    value="{{ old('icon') }}" placeholder="fas fa-globe-europe">
                                                @error('icon')
                                                    <span class="invalid-feedback">{{ $message }}</span>
                                                @enderror
                                            </div>
                                    ...
                                    <div class="form-group">
                                        <button type="submit" class="crancy-btn">
                                            <i class="fa fa-save"></i> {{ __('translate.Save Continent') }}
                                        </button>
                                    </div>
                                </form>
                            </div>
